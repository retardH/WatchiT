...
<template>
  <div
    class="col-auto flex gap-2 rounded-md overflow-hidden"
    :class="
      props.solid
  ...
  >
    <div :class="props.solid ? 'w-5/12 md:w-full h-full' : 'w-full'" class="relative">
      <img
        ...
        class="w-full"
        :class="props.solid ? 'h-full' : 'rounded-md'"
      />
      <div
        :class="props.solid && !props.people && 'hidden md:block'"
        class="absolute -bottom-3 left-2 text-white text-xs"
      >
        <CircularProgress
          :size="30"
          :border-width="3"
    ...
    <div
      ...
      :class="!props.solid ? 'px-0 w-7/12 md:w-full' : 'px-2 w-full'"
    >
      <div class="flex flex-col gap-0.5">
        <h4
          class="text-blue-950 text-sm md:text-base hidden md:block mb-0 text-start"
          style="line-height: 18px"
        >
          {{ sliceWords(props.data.title, 12) }}
        </h4>
        <h4
          class="text-blue-950 text-base w-full flex-1 md:hidden mb-0 text-start"
          style="line-height: 18px"
        >
          {{ props.data.title }}
        </h4>
